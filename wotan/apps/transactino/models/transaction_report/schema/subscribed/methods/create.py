...
class TransactionReportCreateClientResponse(StructureResponse, BaseClientResponse):
  pass

class TransactionReportCreateClientSchema(WithChallengeClientSchema):
  def __init__(self, **kwargs):
    super().__init__(
      **kwargs,
      response=TransactionReportCreateClientResponse,
      children={
...
class TransactionReportCreateSchema(WithOrigin, WithChallenge, StructureSchema):
  def __init__(self, Model, **kwargs):
    ...
    super().__init__(
      **kwargs,
      ...
      client=TransactionReportCreateClientSchema(),
      origin=create_constants.ORIGIN,
      children={
        transaction_report_fields.BLOCKS_TO_INCLUDE: Schema(
          description=(
    ...
    self.response = TransactionReportCreateResponse
  ...
  def responds_to_valid_payload(self, payload, context):
    ...
    blocks_to_include = self.active_response.force_get_child(transaction_report_fields.BLOCKS_TO_INCLUDE).render()
    ...
    if blocks_to_include is not None:
      transaction_report.blocks_to_include = blocks_to_include

    if is_active is not None:
      transaction_report.is_active = is_active

    transaction_report.save()
    transaction_report.process()
